...
P2 = np.dot(K, np.hstack((R[1], T[1].reshape(-1, 1))))
...
def is_in_front(kp1, kp2, R, T, K):
    ...
    homimgpoints = np.transpose(cv2.triangulatePoints(P1,P2, np.transpose(kp1), np.
    transpose(kp2), None))
    imgpoints = homimgpoints[:, :3] / np.repeat(homimgpoints[:, 3], 3).reshape(-1, 3)

    # Put the world coordinates into camera centred coordinates for each camera.
    pts1 = [i for i in imgpoints]
    pts2 = [np.reshape(np.dot(R,i.reshape(3, 1))+T.reshape(3, 1), (1,3) ) [0] for i in
    imgpoints]
